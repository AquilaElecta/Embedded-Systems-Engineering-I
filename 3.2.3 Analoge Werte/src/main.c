...
int main(void)
{
  ...
  while (1)
  {
    ...
    voltage = (voltage_raw * 5000) / 4095;//12-Bit-Auflösung und 5V Referenzspannung
    Decimal_Anzeigen(voltage);
    /* USER CODE BEGIN 3 */
  }
  /* USER CODE END 3 */
}
...
void Decimal_Anzeigen(int zahl){
    uint16_t zahlcode=0;
    zahlcode= (Rechnen_Segment_Code(zahl%10)<<8|0x08);
    Schreiben_74HC595(zahlcode);
    HAL_Delay(2);
    zahlcode= (Rechnen_Segment_Code((zahl/10)%10)<<8|0x04);
    Schreiben_74HC595(zahlcode);
    HAL_Delay(2);
    zahlcode= (Rechnen_Segment_Code((zahl/100)%10)<<8|0x02);
    Schreiben_74HC595(zahlcode);
    HAL_Delay(2);
    zahlcode= (((Rechnen_Segment_Code((zahl/1000)%10)&~(1<<7))<<8)|0x01);// Punkt bei der höchsten Stelle ein
    Schreiben_74HC595(zahlcode);
    HAL_Delay(2);
}
